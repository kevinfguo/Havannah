module aiService{
   ...
   */
  export function createComputerMove(board, playerIndex, alphaBetaLimits) {
  ...
  export function getStateScoreForIndex0(move) { // alphaBetaService also passes playerIndex, in case you need it: getStateScoreForIndex0(move, playerIndex)
    if (move[0].endMatch) {
  ...
  export function getNextStates(move, playerIndex) {
    return gameLogic.getPossibleMoves(move[1].set.value, playerIndex);
  }

  export function getDebugStateToString(move) {
